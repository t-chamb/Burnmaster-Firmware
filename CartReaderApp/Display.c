...
void draw_progressbar(uint32_t processed, uint32_t total, uint8_t line) {
  ...
  if (processed == 0) {
    previous = 0xFF;  // Force redraw
    last_update_count = 0;
  }

  // Calculate current progress with overflow protection
  if (processed >= total) {
    current = steps;
  } else if (total == 0) {
    current = 0;
    ...
    current = (uint8_t)(((uint64_t)processed * steps) / total);
    if (current > steps) current = steps;  // Safety cap
  }

  // Only update if progress changed
  if (current != previous) {
    // Build the progress bar string in memory first
    uint8_t i;
    for (i = 0; i < current && i < steps; i++) {
      bar[i + 1] = '*';  // +1 to skip opening bracket
    }
    
    // Draw entire progress bar at once to reduce flicker
    OledShowString(0, line, bar, 8);
...
void progress_begin(const char* title, const char* status) {
  ...
  OledShowString(0, 0, progress_state.title, 8);
  
  // Display status on line 1 if provided
  if (status && status[0]) {
    OledShowString(0, 1, progress_state.status, 8);
  }
  
  // Initialize progress bar on line 2
  draw_progressbar(0, 100, 2);
}

void progress_update(uint32_t processed, uint32_t total) {
  if (!progress_state.active) return;
  
  // Update progress bar on line 2
  draw_progressbar(processed, total, 2);
}

void progress_complete(const char* message) {
  if (!progress_state.active) return;
  
  // Show completion message on line 4
  if (message && message[0]) {
    OledShowString(0, 4, message, 8);
  }
...
void progress_error(const char* error_msg) {
  ...
  // Show error on line 4 in addition to any progress
  if (error_msg && error_msg[0]) {
    OledShowString(0, 4, error_msg, 8);
  }
  
  progress_state.active = 0;
}

// Simple progress operations for basic tasks
static struct {
...
void simple_progress_begin(const char* operation, uint32_t total) {
  // Don't clear screen, just show operation and initialize progress
  if (operation) {
    // Find first available line (skip if text already there)
    uint8_t line = 0;
    for (line = 0; line < 7; line++) {
      // We'll use line if it's empty (this is simplified)
      break;
    }
    OledShowString(0, line, operation, 8);
    simple_progress.progress_line = line + 1;
  } else {
    simple_progress.progress_line = 0;
  }
  
  simple_progress.total_items = total;
  simple_progress.active = 1;
  draw_progressbar(0, total, simple_progress.progress_line);
}

void simple_progress_update(uint32_t current, uint32_t total) {
  if (!simple_progress.active) return;
  draw_progressbar(current, total, simple_progress.progress_line);
}

void simple_progress_end(void) {
  simple_progress.active = 0;
}

// Display utilities
void display_message(uint8_t line, const char* message) {
  if (line > 7) return; // OLED has 8 lines (0-7)
