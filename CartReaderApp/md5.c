/*
 * MD5 implementation for Burnmaster firmware
...
uint8_t calculateMD5_file(const char* filename, const char* folder, uint8_t digest[16]) {
    ...
    if (folder && strlen(folder) > 0 && strcmp(folder, "root") != 0) {
        res = f_chdir(folder);
        if (res != FR_OK) {
            sprintf(msg, "MD5: Failed to chdir to %s (error %d)\r\n", folder, res);
            soft_uart_send_string(msg);
            return 0;
        }
    }
    
    sprintf(msg, "MD5: Calculating for %s in folder %s\r\n", 
    ...
    if (res != FR_OK) {
        sprintf(msg, "MD5: Failed to open file %s (error %d)\r\n", filename, res);
        soft_uart_send_string(msg);
        return 0;
    }
    ...
    // Print result
    soft_uart_send_string("MD5: Result = ");
    for (int i = 0; i < 16; i++) {
