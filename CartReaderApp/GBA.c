...
void readROM_GBA() 
{
  ...
  // Read rom
  progress_begin("Reading GBA ROM", "Starting...");
  for (int myAddress = 0; myAddress < cartSize; myAddress += 512) {
    // Blink led
    if (myAddress % 16384 == 0)
    {
      LED_RED_BLINK;
      progress_update(myAddress, cartSize);
    }
...
void writeSRAM_GBA(boolean browseFile, unsigned long sramSize, uint32_t pos) 
{
  ...
  if (f_open(&tf, filePath, FA_READ) == FR_OK) 
  {
    ...
    // SRAM writing... - status shown via print messages below
    ...
    progress_begin("Writing SRAM", "Starting...");
    for (unsigned long currAddress = 0; currAddress < sramSize; currAddress += 512) 
    {
      //fill sdBuffer
      UINT rdt;
      ...
      progress_update(currAddress, sramSize);
    }
    // Close the file:
    f_close(&tf);
    progress_complete("SRAM write complete");
    OledShowString(0,6,"finished!",8);
...
void TestSRAM_GBA(unsigned long sramSize)
{
  ...
  OledClear();
  //
  // Start SRAM testing... - status shown on line 7 to avoid conflict
  OledShowString(0,6,"write:",8);

  //
  setAddrOutMode();
  progress_begin("Testing SRAM", "Writing...");
  for (unsigned long currAddress = 0; currAddress < sramSize; currAddress++) 
  {
    ...
    if(wb == 0xFF)progress_update(currAddress, sramSize);
  }
  progress_update(sramSize, sramSize);
  OledShowString(68,6,"read:",8);


  setAddrOutMode();
  uint32_t wErrors = 0;
  progress_set_status("Reading...");
  for (unsigned long currAddress = 0; currAddress < sramSize; currAddress++) {
    // Read byte
    ...
    if(bt == 0xFF)progress_update(currAddress, sramSize);
  }
  progress_update(sramSize, sramSize);
  char msgbuf[64] = {0};
  if(wErrors > 0){
    ...
    progress_complete(msgbuf);
  }else{
    //
    strcpy(msgbuf,"RAM Test ok!");
    progress_complete("RAM Test passed!");
  }
  OledShowString(0,7,msgbuf,8);
}
...
void writeEeprom_GBA(word eepSize) {
  ...
  // Writing eeprom... - status shown on line 7 to avoid conflict
...
void readFLASH_GBA (boolean browseFile, unsigned long flashSize, uint32_t pos)
{
  ...
  progress_begin("Reading Flash", "Starting...");
  for (unsigned long currAddress = 0; currAddress < flashSize; currAddress += 512) 
  {
    LED_RED_BLINK;
    progress_update(currAddress, flashSize);
    for (int c = 0; c < 512; c++) 
    {
      // Read byte
      sdBuffer[c] = readByteFlash_GBA(currAddress + c);
  ...
  progress_complete("Flash read complete");
  f_close(&tf);
...
void writeFLASH_GBA (boolean browseFile, unsigned long flashSize, uint32_t pos)
{
  ...
  // Writing flash... - status shown on line 7 to avoid conflict
...
void verifyFLASH_GBA(unsigned long flashSize, uint32_t pos) 
{
  ...
  // Verify... - status shown on line 7 to avoid conflict
...
void eraseIntel4000_GBA() 
{
  ...
  progress_begin("Erasing Intel 4000", "Starting...");
  for (unsigned long currBlock = 0x0; currBlock < 0x1FFFF; currBlock += 0x8000) {
    ...
    LED_RED_BLINK;
    progress_update(currBlock, lastBlock);
  }
...
void eraseIntel4400_GBA() 
{
  ...
  // Erase 4 blocks with 16kwords each
  progress_begin("Erasing Intel 4400", "Starting...");
  for (unsigned long currBlock = 0x0; currBlock < 0x1FFFF; currBlock += 0x8000) {
    // Unlock Block
    writeWord_GBA(currBlock, 0x60);
    writeWord_GBA(currBlock, 0xD0);
...
void sectorEraseMSP55LV128_GBA(unsigned long lastSector) 
{
  ...
  progress_begin("Erasing MSP55LV128", "Starting...");
...
void sectorEraseMX29GL128E_GBA(unsigned long lastSector) 
{
  ...
  unsigned long currSector;
  progress_begin("Erasing MX29GL128E", "Starting...");
...
void sectorEraseMX29GL128E_GBA_1(unsigned long lastSector) 
{
  // Erase 128 sectors with 128kbytes each
  unsigned long currSector;
  progress_begin("Erasing M29W128GH", "Starting...");
  for (currSector = 0x0; currSector < lastSector; currSector += 0x10000) {
    writeWord_GAB(0xAAA, 0xAA);
    writeWord_GAB(0x555, 0x55);
    writeWord_GAB(0xAAA, 0x80);
...
{
  progress_begin("Writing Intel Flash", "Starting...");
  for (unsigned long currBlock = 0; currBlock < fileSize; currBlock += 0x20000) 
  {
    // Blink led
    LED_BLUE_BLINK;
    progress_update(currBlock, fileSize);
    // Write to flashrom
  ...
  progress_complete("Flash write complete");
}
...
void writeMSP55LV128_GBA(FIL * ptf) 
{
  ...
  progress_begin("Writing MSP Flash", "Starting...");
  for (unsigned long currSector = 0; currSector < fileSize; currSector += 0x10000) 
  {
    // Blink led
    LED_BLUE_BLINK;
    progress_update(currSector, fileSize);
    // Write to flashrom
...
{
  progress_begin("Writing MX29GL128E", "Starting...");
  for (unsigned long currSector = 0; currSector < fileSize; currSector += 0x20000) 
  {
    // Blink led
    LED_BLUE_BLINK;
...
{
  progress_begin("Writing M29W128GH", "Starting...");
  for (unsigned long currSector = 0; currSector < fileSize; currSector += 0x10000) 
  {
    // Blink led
    LED_BLUE_BLINK;
...
boolean verifyFlashrom_GBA() 
{
  ...
  if (f_open(&tf, filePath, FA_READ) == FR_OK) 
  {
    ...
    progress_begin("Verifying Flash", "Starting...");
...
boolean verifyFlashromTest_GBA(uint32_t testSize) 
{
  ...
  for (unsigned long currSector = 0; currSector < testSize; currSector += 131072) 
  {
    // Blink led
    LED_GREEN_BLINK;
    progress_update(currSector, testSize);
    for (unsigned long currSdBuffer = 0; currSdBuffer < 131072; currSdBuffer += 512) 
    {
      readWord_buf_GBA(currSector + currSdBuffer,tbuf,256);
      for (int i = 0; i < (512>>1); i++) 
  ...
  progress_complete("Test verified!");
  if (wErrors == 0) {
    return 1;
  }
  else {
  ...
  if ((strcmp(flashid, "8802") == 0) || (strcmp(flashid, "8816") == 0) || (strcmp(flashid, "227E") == 0) || (strcmp(flashid, "227A") == 0)) 
  {
    ...
    if (f_open(&tf, filePath, FA_READ) == FR_OK) 
    {
      ...
      if (strcmp(flashid, "8802") == 0) {
        // Erasing... - status shown in progress updates
      ...
      //Write flashrom
      // Writing... - status shown on line 4
...
void flashTest_GBA(uint32_t testSize) 
{
  ...
  OledShowString(0,7,"Start ROM Testing...",8);
  idFlashrom_GBA();

  if (strcmp(flashid, "227E") == 0 && romType == 0x1) 
  {
    // Erasing ROM... - status shown in progress
    sectorEraseTest_GBA(testSize - 1);

    //Write flashrom
    // Writing ROM... - status shown in progress
    writeTEST_GBA(testSize);

    // Verify
    // Verifying ROM... - status shown in progress
    resetMX29GL128E_GBA();
    delay(888);
...
static const char GBAMenuItem6[] = "Auto Detect Flash";
static const char GBAMenuItem7[] = "Reset";
static const char* const menuOptionsGBA[] = {GBAMenuItem1, GBAMenuItem2, GBAMenuItem3, GBAMenuItem4, GBAMenuItem5, GBAMenuItem6, GBAMenuItem7};
...
uint8_t gbaMenu() {
  ...
  uint8_t bret = 0;
  unsigned char retMenu = questionBox_OLED("GBA Cart Reader", menuOptionsGBA, 7, 1, 1, 1);
  char tmsg[32] = {0};
  ...
  switch (retMenu)
  {
   ...
   case MENU_1:
    ...
    case MENU_6:
      // Auto Detect Flash
      soft_uart_send_string("GBA Flash: Auto-detecting flash chip\r\n");
      OledClear();
      OledShowString(0, 6, "Detecting Flash...", 8);
      
      // Initialize flash interface
      gba_flash_init();
      
      // Try to detect the flash chip
      uint8_t mfg_id, dev_id;
      if (gba_flash_detect(&mfg_id, &dev_id)) {
        const flash_chip_info_t* chip_info = gba_flash_get_chip_info(mfg_id, dev_id);
        if (chip_info) {
          char info_msg[128];
          sprintf(info_msg, "Found: %s\nMfg: %02X Dev: %02X\nSize: %dKB", 
                  chip_info->name, mfg_id, dev_id, chip_info->size / 1024);
          OledClear();
          OledShowString(0, 0, info_msg, 8);
          OledShowString(0, 3, "Press OK to flash", 8);
          OledShowString(0, 4, "or Cancel to exit", 8);
          ...
          while(1) {
            ...
            if (btn == BTNOK) {
              ...
              soft_uart_send_string("GBA Flash: Launching file browser\r\n");
              fileBrowser("/", "Select ROM file:");
              
              if (strlen(filePath) > 0) {
                OledClear();
                // Flashing... - status shown in specific operations
                
                // Erase chip
                OledShowString(0, 6, "Erasing...", 8);
                if (!gba_flash_erase_chip(chip_info)) {
                ...
                if (f_open(&tf, filePath, FA_READ) == FR_OK) {
                  ...
                  OledShowString(0, 6, "Writing...", 8);
                  
                  progress_begin("Writing GB Flash", "Starting...");
                  while (bytes_written < file_size && bytes_written < chip_info->size) {
                    UINT bytes_read;
                    f_read(&tf, buffer, sizeof(buffer), &bytes_read);
                    if (bytes_read == 0) break;
                    
                    if (!gba_flash_write_buffer(chip_info, bytes_written, buffer, bytes_read)) {
                      f_close(&tf);
                      print_Error("Write failed!", true);
                    }
                    
                    bytes_written += bytes_read;
                    progress_update(bytes_written, file_size);
                    LED_GREEN_BLINK;
                  }
                  ...
                  OledShowString(0, 6, "Verifying...", 8);
                  // Reset and verify
                  gba_flash_reset();
                  ...
                  if (f_open(&tf, filePath, FA_READ) == FR_OK) {
                    ...
                    progress_begin("Verifying GB Flash", "Starting...");
                    while (bytes_written < file_size && bytes_written < chip_info->size) {
                      UINT bytes_read;
                      f_read(&tf, buffer, sizeof(buffer), &bytes_read);
                      if (bytes_read == 0) break;
                      
                      if (!gba_flash_verify(chip_info, bytes_written, buffer, bytes_read)) {
                        verify_ok = false;
                        break;
                      }
                    ...
                    progress_complete(verify_ok ? "Verified OK!" : "Verification failed");
                    
                    f_close(&tf);
                    
                    if (verify_ok) {
                      OledShowString(0, 6, "Flash complete!", 8);
                    } else {
          ...
          char err_msg[64];
          sprintf(err_msg, "Unknown chip:\nMfg:%02X Dev:%02X", mfg_id, dev_id);
          OledShowString(0, 2, err_msg, 8);
        }
      } else {
...
uint32_t gba_get_rom_size(void) {
  // Read cart size from header at 0x80000AC
  uint32_t romSize = readWord_GBA(0x80000AC);
  
  // Common GBA ROM sizes
  switch(romSize) {
    case 0x00: return 0x200000;   // 2MB
    case 0x01: return 0x400000;   // 4MB  
    case 0x02: return 0x800000;   // 8MB
    case 0x03: return 0x1000000;  // 16MB
    case 0x04: return 0x2000000;  // 32MB
    default:
      // Try to detect based on mirroring
      if (romSize > 0x04) return 0x2000000; // Default to max
      return 0x400000; // Default to 4MB
  }
}
...
const char* gba_get_save_type_string(uint8_t saveType) {
  switch(saveType) {
    case 0: return "None";
    case 1: return "EEPROM 4K";
    case 2: return "EEPROM 64K"; 
    case 3: return "SRAM 256K";
    case 4: return "FLASH 512K";
    case 5: return "FLASH 1M";
    case 6: return "SRAM 512K";
...
uint32_t gba_get_save_size(uint8_t saveType) {
  switch(saveType) {
    ...
    case 1: return 512;      // EEPROM 4K
    case 2: return 8192;     // EEPROM 64K
    case 3: return 32768;    // SRAM 256K
    case 4: return 65536;    // FLASH 512K
    case 5: return 131072;   // FLASH 1M
    case 6: return 65536;    // SRAM 512K
    default: return 0;
  }
}

void gba_set_address(uint32_t address) {
  // Set the address on the GBA cart bus
  // Split across three GPIO ports
  GPIO_OCTL(ADDR_1) = (GPIO_OCTL(ADDR_1)&0xFFFF0000) | (address & 0xFFFF);
  GPIO_OCTL(ADDR_2) = (GPIO_OCTL(ADDR_2)&0xFFFFF0FF) | ((address >> 8) & 0x0F00);
  GPIO_OCTL(ADDR_3) = (GPIO_OCTL(ADDR_3)&0xFF0000FF) | ((address >> 8) & 0xFF00);
}
